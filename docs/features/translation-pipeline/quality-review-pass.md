   ...
   - A correction is proposed → restored via `tokenReplacer.Restore`, then run through the
     **validation gate**: `TranslationWorkflow.EvaluateRules` (the same shared rule list a normal
     ...
     - Gate passes → **accepted immediately**: `QcStatus = Corrected`, `QcTranslated` = the
      ...
      score here does **not** trigger another retry; a validated correction remains available for
      packaging even when its confidence score is below the review threshold.
...
1. If fresh and `Utility.QualityReviewHelpers.PassesQcScoreGate(QcQualityScore, QcDefectCategory,
  ...
  remains available for packaging; this is reported as `QcRejected`, not `RawFallback`.
...
"did QC's correction introduce this defect" from "was it already there before QC touched it".
...
- **`ResetCorrectedQcState(workingDirectory, textFiles)`** — resets every column currently at
  ...
  overcorrects scores upward instead of down). Leaves `Passed`/
...
- **`ResetAllQcState`** — wipes **every** column's Qc* state back to `NotReviewed`
  ...
  never triggers any re-review on its own. Use this when a `Passed` column was reviewed before a
  prompt or scoring rule changed, or a `Corrected`
...
- **`LowScoreSample`** — rows with no `Reason`, just `Score` below `minAcceptableScore`. The
  ...
  self-rated confidence, which is frequently miscalibrated. A small,
