# Packaging workflows and file types
...
> today**. For the QC pass that packaging shares a score-gate/freshness
> mechanism with, see [`quality-review-pass.md`](../translation-pipeline/quality-review-pass.md)
> (this file links to it rather than re-explaining QC internals). For historical incident context,
> see [the packaging QC raw-fallback investigation](../../investigations/packaging-qc-raw-fallback.md).
...
- `Workflow/DynamicStringsCecilWorkflow.PackageDynamicStringsCecilAsync` — legacy `TextFileType.DynamicStrings`.
...
correction introduce this defect" from "was it already there before QC touched it".
...
`TextFileType.LocalTextString` does not select a packaging workflow. It is a translation-time
guard for game-object references; package that file through the workflow for its actual source
format. See the [LocalTextString guide](../text-handling/local-text-string.md).
...
### When QC rejects a correction

QC rejection keeps ordinary `Translated` text. See [the packaging QC raw-fallback investigation](../../investigations/packaging-qc-raw-fallback.md).
